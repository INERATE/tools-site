...
| 15 | Remove Password from PDF · WEBP to PNG · PNG to WEBP | ✅ shipped, verified live in a real browser (Playwright, locked PDF + real .webp round-tripped correctly) — Remove Password reuses `/unlock-pdf`'s `useUnlock` verbatim; the two WEBP pages reuse `useLockedImageFormat`. Found and fixed a real root-cause bug along the way: `useImageBatch` (shared by every image tool) hardcoded a JPG/PNG-only filter, so WEBP input was silently rejected — fixed once in the shared hook rather than special-casing the new pages, which also makes WEBP a valid input everywhere else in the suite that shares this hook |
| 16 | JPG to WEBP · WEBP to JPG · PDF to PNG | ✅ shipped — same zero-new-logic templating as Phases 13-15. Also fixed two real navbar bugs reported live: "Live Simulator" used a bare `#demo-stage` hash that only resolves on the homepage, silently doing nothing from any tool page (fixed to `/#demo-stage`); and the condensed-pill nav state kept all four links + Launch PDF in a 780px-capped pill that couldn't fit them, so they overflowed the glass pill and spilled text onto the hero content below (screenshot-verified) — fixed by collapsing the link row away on condense (matches the GrowthCharters reference: only logo + one CTA survive the shrink) and narrowing the pill's `maxWidth` to fit just that. Launch PDF also picked up the existing `.shimmer` sheen class already used on every other primary CTA in the suite |
| 17 | CSV to Excel · Excel to CSV · Image to Base64 | ✅ shipped, verified live in a real browser (Playwright, real .csv/.xlsx/.png round-tripped correctly) — the two spreadsheet tools reuse `xlsx` (SheetJS), already a bundle dependency for excel-to-pdf, both directions (`XLSX.read(text, {type:"string"})` parses CSV natively — no hand-rolled parser needed); Image to Base64 is a genuinely new zero-dependency tool (`FileReader.readAsDataURL`), useful for pasting a data: URI straight into CSS/HTML/JSON. Also fixed a real navbar bug reported live: the condensed pill background stayed translucent instead of solidifying, so it never read as "arrived" once scrolled — added a `data-condensed` CSS variant so the pill goes near-opaque on condense while the bar state stays translucent over the hero |
| 17b | Crop Image: shape masks (Circle · Rounded · Custom SVG path) | ✅ shipped, verified pixel-level (Python/PIL alpha checks on real downloaded output, not just a screenshot) — the crop rectangle can now be masked to a circle, an adjustable rounded-rect, or an arbitrary SVG path pasted on a 0-100 viewBox (`Path2D` + `DOMMatrix.scale` maps it onto the actual crop size). Any non-rectangle shape forces PNG output regardless of the source format, since the cutout needs a real alpha channel — verified corners are alpha 0 and the shape interior is alpha 255 on the actual downloaded file. Live preview mirrors the export exactly: built-in shapes via CSS `clip-path`, Custom via an inline SVG `clipPath` using the same 0-100 viewBox so preview and export never disagree |
| 18 | Watermark PDF · Extract PDF Pages · Delete PDF Pages | ✅ shipped, verified live in a real browser (Playwright, real 3-page PDF round-tripped correctly + `pypdf` checked actual output byte counts and text) — Watermark PDF was a real gap (watermark-image existed for photos, nothing stamped a PDF): new `watermarkPdf()` in `app/lib/watermark-pdf.ts`, pdf-lib `drawText` per page, tiled diagonal / center / corner, verified the watermark text is actually embedded in the output (not just a screenshot). Extract PDF Pages and Delete PDF Pages are the same zero-new-logic templating as Phases 13-16: both reuse `/pdf-split`'s `usePageBoard` + `useAssembleRun` + `ToolBoard` engine verbatim — Extract keeps `pdf-split`'s "keep pages" QuickPick almost as-is under new SEO copy targeting the higher-volume "extract pdf pages" query; Delete PDF Pages adds one small inverse-logic QuickPick (parses "pages to remove", computes the complement to keep) so the tool reads naturally for the "delete pdf pages" search intent instead of asking the user to think in terms of what to keep |
