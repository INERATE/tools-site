...
export function Dropzone({ onFiles }: { onFiles: (files: File[]) => void }) {
  return (
    <label className={ZONE}>
      ...
      />
      <span className="block text-[15px] font-medium">Click to add PDF files</span>
      <span className="mt-1.5 block text-[13px] text-[var(--text-dim)]">
        PDFs only — they never leave this tab
      </span>
    </label>
  );
}
