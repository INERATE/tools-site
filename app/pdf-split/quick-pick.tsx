...
const PRESETS = (n: number) => [
  ...
  ...(n > 1 ? [{ label: "Without the first", value: `2-${n}` }] : []),
];
...
/** Types a range and hands the board the pages to keep. */
export function QuickPick({ total, onKeep }: { total: number; onKeep: (pages: number[]) => void }) {
  ...
  const apply = (value: string) => {
    setText(value);
    try {
      onKeep(parseRanges(value, total));
      setBad(null);
    } catch (e) {
  ...
  return (
    <div className="glass mb-5 rounded-2xl p-4">
      <label htmlFor="keep" className="mb-2 block text-[13px] font-medium">
        Keep pages
      </label>
      <div className="flex gap-2">
        <input
          ...
          placeholder={`e.g. 1-3, 5 — of ${total}`}
